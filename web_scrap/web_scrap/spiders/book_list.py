...
class IntroSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = [
            'http://books.toscrape.com/catalogue/page-{x}.html'.format(x=x) for x in range(1, 50)
            # x denotes page number
        ]
    ...
    def parse(self, response):
        ...
        image_link = response.css(
            'article.product_pod > div.image_container > a > img::attr(src)').extract()  # accessing the titles
